...
if (true) { 
  ...
  try{
      ...
      const contractLottery = new ethers.Contract(contractAddress, contractABI, signer);
      const tx = await contractLottery.winnerPick(item[0]);
...
const paySeller = async (item) => {
    if(calculateRemainingTime(item[3]) > 0)
        ...
        try{
          ...
          const signer = await provider.getSigner();
          const contractLottery = new ethers.Contract(contractAddress, contractABI, signer);
          const tx =  await contractLottery.paySeller(58);
          console.log(`Transaction sent: ${tx.hash}`);
...
<button className="btn btn-secondary btn-sm" onClick={() => paySeller(data)} style={{ flexGrow: 1 , marginRight : 5}}>Pay Seller</button>
  <button className="btn btn-secondary btn-sm" onClick={() => Lottery(data)} style={{ flexGrow: 1 , marginRight : 5}}>Lottery</button>
</div>
        </div>
      </li>
    ))}
